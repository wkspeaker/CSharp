...
namespace LangrisserTools.Core.Services
{
    ...
    public abstract class DataService<T> : IDataService<T> where T : class, new()
    {
        ...
        #region IDataService<T> 简单实现封装默认文件名
        // 实现接口，默认使用方法中约定的文件名需要在派生类中实现
        public virtual Task<T> LoadDataAsync()
        {
            throw new NotImplementedException("Derived classes should implement LoadDataAsync with default file name");
        }

        public virtual Task SaveDataAsync(T data)
        {
            throw new NotImplementedException("Derived classes should implement SaveDataAsync with default file name");
        }

        public virtual T LoadData()
        {
            throw new NotImplementedException("Derived classes should implement LoadData with default file name");
        }

        public virtual void SaveData(T data)
        {
            throw new NotImplementedException("Derived classes should implement SaveData with default file name");
        }
        #endregion
    }
}
